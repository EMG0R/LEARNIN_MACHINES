...
_HERE       = Path(__file__).resolve().parent          # lib/combined_app/
RESOURCES   = _HERE.parent.parent                       # RESOURCES/
CHECKPOINTS = RESOURCES / "checkpoints"
REPO        = RESOURCES                                 # legacy alias

HAND_SEG_CKPT    = CHECKPOINTS / "hand_seg_v7.pt"
GESTURE_CKPT     = CHECKPOINTS / "gesture_v7.pt"
FACE_DET_CKPT    = CHECKPOINTS / "face_det_v1.pt"
FACE_PARTS_CKPT  = CHECKPOINTS / "face_parts_v1.pt"
EMOTION_CKPT     = CHECKPOINTS / "emotion_v1.pt"
...
# In TD bundle the checkpoint lives next to the others under RESOURCES/checkpoints/
_OBJ_CANDIDATES = ["obj_seg_v5.pt", "best.pt", "obj_seg_integration.pt"]
OBJ_CKPT = next((CHECKPOINTS / n for n in _OBJ_CANDIDATES if (CHECKPOINTS / n).exists()),
                CHECKPOINTS / "obj_seg_v5.pt")
OBJ_SIZE = 320
PERSON_MIN_AREA = 0.05
