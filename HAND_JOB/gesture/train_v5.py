"""Gesture v5 — aggressive augmentation tuned for webcam deployment, not HaGRID test set.

v4 hit F1 0.96 on HaGRID test but failed in the live app. Diagnosis: the model
overfit to HaGRID's specific lighting/camera/crop distribution. Fix is not a
better model or training trick — it's augmentation that simulates deployment:

  1. VERY WIDE BBOX PADDING 0.0–1.0 (was 0.05–0.25) — hand can fill entire crop
     (tight) or be small within it (loose). Matches arbitrary distance from cam.
  2. RandomResizedCrop scale 0.3–1.0 (was 0.7–1.0) — strong scale variance on
     top of the padded crop. Hand size in final image varies dramatically.
  3. RandomAffine rotation ±20°, translate ±10%, scale 0.8–1.2 — compound
     geometric variance. Hand rarely centered or one specific size.
  4. ColorJitter doubled (brightness/contrast/sat 0.4, hue 0.1) — webcam lighting
     wildly different from studio.
  5. GaussianBlur p=0.35 wider sigma (0.1–2.0) — motion/focus blur.
  6. Light RandomErasing (0.02–0.08, p=0.15) — partial occlusion / self-shadow.
  (No grayscale — deployment is always color.)

Still NO AMP/EMA/MixUp/LabelSmoothing — those are what killed v3.

Run from HAND_JOB/gesture:
    ../.venv/bin/python -u train_v5.py
"""
...
RUN_TAG   = os.environ.get("RUN_TAG", "v5")
SEED      = 42
...
# ---------------- TRANSFORMS (deployment-matched, aggressive) ----------------
def build_tf(img_size, mode):
    if mode == "eval":
        return T.Compose([T.Resize((img_size, img_size)), T.ToTensor(),
                          T.Normalize(IMAGENET_MEAN, IMAGENET_STD)])
    return T.Compose([
        T.RandomResizedCrop(img_size, scale=(0.3, 1.0), ratio=(0.75, 1.33)),
        T.RandomHorizontalFlip(p=0.5),
        T.RandomAffine(degrees=20, translate=(0.10, 0.10), scale=(0.8, 1.2),
...
class HagridDS(Dataset):
    """Training uses WIDE padding variance (0.05–0.50) to match live-app mask crops."""
    ...
    def __getitem__(self, i):
        ...
        side = max(pw, ph)
        # KEY CHANGE: train sees pads from ultra-tight (0%) to very loose (100%).
        # Compounds with RandomResizedCrop → hand occupies anywhere from ~30% to
        # ~100% of final image. This is the main driver of size robustness.
        pad = side * (random.uniform(0.0, 1.0) if self.mode == "train" else 0.15)
